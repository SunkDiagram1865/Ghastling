...
const requiredInvariants = [
	['product name', tauriConfig.productName === 'Ghastling Launcher'],
	['bundle identifier', tauriConfig.identifier === 'com.cysunk.ghastling'],
	[
		'deep-link scheme',
		tauriConfig.plugins?.['deep-link']?.desktop?.schemes?.includes('ghastling') === true,
	],
	[
		'User-Agent format',
...
if (failures.length > 0) {
	console.error(`Ghastling brand guard failed:\n${failures.join('\n')}`)
	process.exit(1)
}

console.log('Ghastling brand guard passed.')
