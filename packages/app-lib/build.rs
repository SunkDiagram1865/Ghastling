...
fn build_java_jars() {
    ...
    if env::var_os("JAVA_HOME").is_none() {
        if let Some(jdk_home) = find_system_jdk() {
            println!(
                "cargo::warning=JAVA_HOME not set, using detected JDK: {}",
                jdk_home.display()
            );
    ...
    let exit_status = cmd.status().expect("Failed to wait on Gradle build");
...
fn find_system_jdk() -> Option<PathBuf> {
    let program_files = env::var_os("ProgramFiles")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(r"C:\Program Files"));
    let java_dir = program_files.join("Java");
    let entries = fs::read_dir(&java_dir).ok()?;
    let mut jdirs: Vec<PathBuf> = entries
        .filter_map(|e| e.ok())
        .filter_map(|e| {
            let path = e.path();
            let name = e.file_name().to_string_lossy().to_string();
            if name.starts_with("jdk-")
                && path.join("bin").join("javac.exe").exists()
            {
                Some(path)
            } else {
                None
